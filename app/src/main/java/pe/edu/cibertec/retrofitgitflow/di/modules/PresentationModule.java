...
public class PresentationModule {
    ...
    @PerActivity
    @Provides
    IMainInteractor provideMainInteractor(IPostRepository postRepository,
                                          @Named("ui_thread") Scheduler uiThread,
                                          @Named("executor_thread") Scheduler executorThread){
        return new MainInteractorImpl(postRepository, uiThread, executorThread);
    }

    @PerActivity
    @Provides
    IPostDetailInteractor providePostDetailInteractor(IPostRepository postRepository,
                                                      @Named("ui_thread") Scheduler uiThread,
                                                      @Named("executor_thread") Scheduler executorThread){
        return new PostDetailInteractorImpl(postRepository,uiThread,executorThread);
    }
}
